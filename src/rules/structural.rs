...
//! SLOP-X002 bullet-heavy commit body, SLOP-X003 oversized body,
...
pub const HANDLED: &[&str] = &["SLOP-X001", "SLOP-X002", "SLOP-X003", "SLOP-X004"];
...
pub fn evaluate(
    ...
    if let Some(idx) = super::active(cp, config, "SLOP-X002") {
        if let FormatData::Commit(split) = &prepared.format {
            let rule = &cp.pkg.rules[idx];
            let min_lines = super::param_i64(rule, "min_body_lines").unwrap_or(10) as usize;
            let min_pct = super::param_i64(rule, "min_bullet_line_pct").unwrap_or(50) as usize;
            let body = &prepared.text[split.body.clone()];
            let mut lines = 0usize;
            let mut bullet_lines = 0usize;
            for lr in line_ranges(body) {
                let line = body[lr].trim_start();
                if line.is_empty() {
                    continue;
                }
                lines += 1;
            ...
            if lines >= min_lines && bullet_lines * 100 >= min_pct * lines {
                hits.push(Hit::new(idx, split.body.clone()));
            }
        }
    }

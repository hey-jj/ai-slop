...
pub const F_SUBJECT: u8 = 4;
pub const F_BODY: u8 = 8;
pub const F_TRAILER: u8 = 16;
...
pub fn build_doc(prepared: &Prepared, _config: &Config) -> Result<Doc, AnalysisError> {
    let doc = match &prepared.format {
        ...
        FormatData::Commit(split) => build_commit(&prepared.text, split),
        FormatData::Manifest(info) => build_manifest(&prepared.text, info),
    };
    Ok(doc)
}
...
fn build_markdown(src: &str) -> Doc {
    ...
    for (event, range) in parser.into_offset_iter() {
        ...
        // artifact (it emits one between the wrapped lines of a tab-indented
            ...
            Event::Start(tag) => {
                ...
                match tag {
                    ...
                    Tag::Emphasis | Tag::Strong => {
                        ...
                        if item_fresh && matches!(tag, Tag::Strong) {
...
fn build_commit(src: &str, split: &crate::input::CommitSplit) -> Doc {
    let mut doc = Doc::default();
    let lines = line_ranges(src);
    for lr in &lines {
        let line = &src[lr.clone()];
        ...
        let flags = if lr.start >= split.trailers.start && split.trailers.start < split.trailers.end
        {
            F_TRAILER
        } else if lr.start >= split.body.start && lr.start < split.body.end {
            F_BODY
        } else if lr.start >= split.subject.start && lr.end <= split.subject.end {
            F_SUBJECT
        } else {
            F_BODY
        };
...
fn build_manifest(src: &str, info: &crate::input::ManifestInfo) -> Doc {
    let mut doc = Doc::default();
    if let Some(inner) = &info.description_inner {
        doc.ops.push(NormOp::Block);
        doc.ops.push(NormOp::Text {
            range: inner.clone(),
            flags: 0,
        });
        doc.stats.word_count += count_words(&src[inner.clone()]);
    } else if let (Some(span), Some(value)) = (&info.description_span, &info.description) {
        doc.ops.push(NormOp::Block);
        doc.stats.word_count += count_words(value);
        doc.ops.push(NormOp::TextOwned {
            range: span.clone(),
            content: value.clone(),
            flags: 0,

...
impl Profile {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Profile::PublicBugReport => "public-bug-report",
            Profile::CommitMessage => "commit-message",
    ...
    pub fn default_format(self) -> InputFormat {
        match self {
            Profile::CommitMessage => InputFormat::Commit,
            Profile::CargoMetadata => InputFormat::Manifest,
    ...
    pub fn supported_formats(self) -> &'static [InputFormat] {
        match self {
            Profile::CommitMessage => &[InputFormat::Commit],
            Profile::CargoMetadata => &[InputFormat::Manifest],
            _ => &[InputFormat::Markdown, InputFormat::Text],
        }
    }
}
...
impl InputFormat {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            InputFormat::Commit => "commit",
            InputFormat::Manifest => "manifest",
        }
    }
    ...
    pub fn from_str(s: &str) -> Option<InputFormat> {
        match s {
            ...
            "commit" => Some(InputFormat::Commit),
            "manifest" => Some(InputFormat::Manifest),
            _ => None,
        }
    }
}
...
pub struct Deployment {
    ...
    pub expected_release_body: Option<String>,
    /// Narrows the scrub-word list. Terms outside the embedded list are
    /// ignored, since the global automaton is compiled from the package.
    pub scrub_overrides: Option<Vec<String>>,
...
/// Commit-message field, used for per-field rule stances.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Field {
    Whole,
    Subject,

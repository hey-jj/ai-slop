...
/// approval with empty or under-privileged waivers cannot pass an artifact
...
pub fn verify(input: &[u8], approval: &Approval, now_unix: i64) -> VerifyOutcome {
    ...
    let want = approval
        .artifact_sha256
        .strip_prefix("sha256:")
        .unwrap_or(&approval.artifact_sha256);
    if got != want {
        problems.push(format!(
            "artifact hash mismatch: payload is {got}, approval says {want}"
        ));
    }

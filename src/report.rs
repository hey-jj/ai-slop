...
fn suggestion_for(rule_id: &str, span: &Range<usize>) -> Option<Suggestion> {
    let replace_with = match rule_id {
        "SLOP-M001" => ", ",
        "SLOP-M002" => ".",
        "SLOP-M003" | "SLOP-M006" | "SLOP-T001" | "SLOP-I001" => "",
        _ => return None,
    };
    Some(Suggestion {
        start: span.start,
...
pub fn assemble(
    ...
    for hit in &hits {
        ...
        let stance = rule.stance(config.profile, hit.field);
        ...
        let suggestion = if config.suggest && !hit.force_hint {
            suggestion_for(&rule.id, &hit.span)
        } else {
            None
        };
    ...
    let note = if config.suggest {
        Some(
            "applying any suggestion changes the artifact hash and invalidates any approval"
                .to_string(),
        )
    } else {
        None

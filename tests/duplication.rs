...
fn u001(report: &ai_slop::Report) -> Vec<&ai_slop::Finding> {
    report
        .findings
        .iter()
...
fn u001_worst_case_shape_stays_inside_the_memory_budget() {
    ...
    let config = ai_slop::Config::new(Profile::InternalDoc);
...
/// api-docs and changelog are off: rustdoc boilerplate and changelog entry
/// stems repeat by convention.
#[test]
fn u001_is_off_where_repetition_is_structural() {
    let text = format!("{PARA} Filler words pad this line out considerably.\n\n{PARA}\n");
    for profile in [Profile::ApiDocs, Profile::Changelog] {
        let report = run(&text, profile);
        assert!(
            !has_rule(&report, "SLOP-U001"),
            "U001 must be off under {}",
            profile.as_str()
        );
    }
}

/// A normal README-shaped fixture produces zero U001 findings, and repeated
/// analysis of a duplicated document is byte-identical.
#[test]
fn u001_clean_fixture_and_determinism() {
    let fixture = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/fixtures/golden/lightningcss-gamut-mapping-early-return-desaturated-fallback.md"
    );
    let text = std::fs::read_to_string(fixture).unwrap();
    let report = run(&text, Profile::PublicBugReport);
    assert!(!has_rule(&report, "SLOP-U001"), "clean fixture fired U001");

    let dup = format!("{PARA} Filler follows in this line.\n\n{PARA}\n");
    let config = ai_slop::Config::new(Profile::InternalDoc);
    let a = serde_json::to_string(&ai_slop::analyze(dup.as_bytes(), &config).unwrap()).unwrap();
    let b = serde_json::to_string(&ai_slop::analyze(dup.as_bytes(), &config).unwrap()).unwrap();

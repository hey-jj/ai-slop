...
use ai_slop::{analyze, Config, Profile};
...
proptest! {
    ...
    fn analyze_never_panics_on_arbitrary_bytes(bytes in proptest::collection::vec(any::<u8>(), 0..2048)) {
        for profile in [Profile::Readme, Profile::CommitMessage, Profile::CargoMetadata] {
    ...
    fn r7_never_false_trips_a_real_finding(text in "\\PC{0,400}") {
        for profile in [Profile::Readme, Profile::PublicBugReport, Profile::InternalDoc] {

...
#[test]
fn non_ascii_before_heading_keeps_exact_heading_span() {
    // The document MUST begin with five single-byte chars then a two-byte char
    // (İ at bytes 5..7) so the heading-local "Impact" span (0..6) lands mid-char
    // when wrongly widened against the full source. A full-source widen
    // emits "Impact " (7 bytes); the coordinate-correct widen keeps "Impact" (6).
    let text = "aaaaaİ ordinary words to fill this opening line here.\n\n# Impact now\n\nThe report body describes the observed failure in plain prose for the reader.\n";
    let report = run(text, Profile::PublicBugReport);
    let s002 = report
        .findings
        .iter()
        .find(|f| f.rule_id == "SLOP-S002")
        .expect("verdict-heading S002 must fire on the Impact heading");
    assert_eq!(
        snippet(s002),
        "Impact",
        "heading span must be exactly the word"
    );
    assert_eq!(s002.spans[0].end - s002.spans[0].start, 6);
}

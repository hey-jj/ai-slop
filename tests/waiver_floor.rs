...
use ai_slop::waiver::{Approval, Waiver, WaiverSpan};
use ai_slop::{analyze, AnalysisError, Config, Profile, VerifyOutcome, WaiverAuthority};

fn approval_with(waiver: Waiver, payload: &[u8]) -> Approval {
    Approval {
        artifact_sha256: ai_slop::input::sha256_hex(payload),
        policy_digest: ai_slop::policy_digest(),
        profile: "readme".to_string(),
        approved_at: None,
...
fn verify_rejects_agent_waivers_on_the_floor() {
    ...
    for rule_id in ["SLOP-A001", "SLOP-A002", "SLOP-J001"] {
        ...
        match ai_slop::verify(payload, &approval, 0) {
...
fn verify_rejects_waivers_on_fail_closed_states_from_any_signer() {
    ...
    for rule_id in ["instrumentation_error", "unsupported_input"] {
        ...
        assert!(
            matches!(
                ai_slop::verify(payload, &approval, 0),
...
fn verify_accepts_a_clean_artifact_with_a_matching_hash() {
    ...
    let approval = Approval {
        artifact_sha256: ai_slop::input::sha256_hex(payload),
        policy_digest: ai_slop::policy_digest(),
        profile: "internal-doc".to_string(),
        approved_at: None,
        approver: Some("test".to_string()),
        approver_kind: Some("human".to_string()),
        demote: vec![],
...
fn verify_fails_on_hash_digest_or_expiry_mismatch() {
    ...
    assert!(matches!(
        ai_slop::verify(b"mutated payload", &approval, 0),
        VerifyOutcome::Mismatch(_)
    ));
    ...
    assert!(matches!(
        ai_slop::verify(payload, &bad_digest, 0),
        VerifyOutcome::Mismatch(_)
    ));
    ...
    expired.profile = "readme".to_string();
    assert!(matches!(
        ai_slop::verify(payload, &expired, 4_102_444_800),
        VerifyOutcome::Mismatch(_)
    ));
}
...
// Hole 1: an empty-waiver approval whose hash matches a dirty artifact must be
// rejected, because verify now re-runs the linter.
#[test]
fn verify_rejects_empty_waiver_over_a_dirty_artifact() {
    let payload = b"We delve into this."; // SLOP-A001 violation, blocking.
    let approval = approval("internal-doc", payload, vec![]);
    let problems = rejects(ai_slop::verify(payload, &approval, 1_000_000_000));
    assert!(
...
fn verify_binds_analysis_to_the_approval_profile() {
    let payload = b"I think this fixes the bug."; // F001 blocking under public-bug-report, clean under internal-doc.
    let strict = approval("public-bug-report", payload, vec![]);
    let problems = rejects(ai_slop::verify(payload, &strict, 1_000_000_000));
    assert!(
        problems.iter().any(|p| p.contains("SLOP-F001")),
        "strict profile not applied: {problems:?}"
    );
    let lax = approval("internal-doc", payload, vec![]);
    assert_eq!(
        ai_slop::verify(payload, &lax, 1_000_000_000),
        VerifyOutcome::Verified,
...
fn verify_rejects_untrusted_signer_on_the_human_only_floor() {
    ...
    for (payload, rule_id, (start, end)) in cases {
        for signer in [None, Some("bogus")] {
            ...
            let approval = approval("internal-doc", payload, vec![w]);
            let problems = rejects(ai_slop::verify(payload, &approval, 1_000_000_000));
            assert!(
                problems.iter().any(|p| p.contains(rule_id)),
...
fn verify_ignores_expired_and_span_mismatched_waivers() {
    ...
    let problems = rejects(ai_slop::verify(payload, &expired, 1_700_000_000));
    assert!(
        problems.iter().any(|p| p.contains("SLOP-A001")),
    ...
    let problems = rejects(ai_slop::verify(payload, &mismatched, 1_000_000_000));
    assert!(
        problems.iter().any(|p| p.contains("SLOP-A001")),
...
fn verify_accepts_valid_human_waiver_on_a_human_only_rule() {
    ...
    assert_eq!(
        ai_slop::verify(payload, &approval, 1_000_000_000),
        VerifyOutcome::Verified
    );
}
...
fn demoting_violation_tier_or_j001_is_a_usage_error() {
    let mut config = Config::new(Profile::InternalDoc);
...
fn verify_carries_approval_demote_and_agrees_with_the_gate() {
    ...
    let mut gate = Config::new(Profile::InternalDoc);
    ...
    assert_eq!(
        ai_slop::verify(payload, &with_demote, 1_000_000_000),
        VerifyOutcome::Verified,
...
fn verify_demote_cannot_lower_the_floor() {
    ...
    let mut violation = approval("internal-doc", payload, vec![]);
    violation.demote = vec!["SLOP-A001".to_string()];
    let problems = rejects(ai_slop::verify(payload, &violation, 1_000_000_000));
    assert!(
        problems.iter().any(|p| p.contains("SLOP-A001")),
    ...
    let mut j001 = approval("internal-doc", payload, vec![]);
    j001.demote = vec!["SLOP-J001".to_string()];
    let problems = rejects(ai_slop::verify(payload, &j001, 1_000_000_000));
    assert!(
...
fn verify_rejects_approval_with_incomplete_embedded_waiver() {
    ...
    let problems = rejects(ai_slop::verify(payload, &incomplete, 1_000_000_000));
    assert!(
        problems
            .iter()

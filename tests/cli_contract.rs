...
fn bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_ai-slop"))
}
...
fn incomplete_waiver_file_is_usage_error_complete_waiver_still_works() {
    let dir = std::env::temp_dir().join("ai-slop-cli-test-waivers");
    std::fs::create_dir_all(&dir).unwrap();
...
#[test]
fn unsupported_format_for_profile_is_usage_error() {
    let (code, _, _) = run_stdin(
        &[
            "check",
            "--profile",
...
fn suggest_only_annotates_mechanical_rules() {
    let (_, stdout, _) = run_stdin(
        &["check", "--profile", "internal-doc", "--suggest", "-"],

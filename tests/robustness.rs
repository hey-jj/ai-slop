...
use ai_slop::{analyze, AnalysisError, Config, Profile};

fn bin() -> &'static str {
    env!("CARGO_BIN_EXE_ai-slop")
}
...
fn timed_analyze(input: &str) -> (Result<ai_slop::Report, AnalysisError>, Duration) {
...
fn closed_stdout_never_panics_on_any_output_path() {
    let dir = std::env::temp_dir().join("ai-slop-closed-stdout");
    ...
    let cases: Vec<(Vec<&str>, &[u8], i32)> = vec![
        ...
        // Success report path.
        (
            vec!["check", "--profile", "internal-doc", "-"],
            b"Plain.\n",
            0,
        ),
...
fn report_into_early_closing_pipe_keeps_exit_code() {
    ...
    let mut child = Command::new("bash")
        .arg("-c")
        .arg(r#""$0" check --profile internal-doc - | head -c 1 >/dev/null; exit "${PIPESTATUS[0]}""#)
        .arg(bin())
        .stdin(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
fn second_positional_is_usage_error() {
    let (code, stdout, stderr) =
        run_stdin(&["check", "--profile", "internal-doc", "a.md", "b.md"], b"");
...
fn wrong_type_config_value_is_usage_error() {
    let dir = std::env::temp_dir().join("ai-slop-config-types");
    std::fs::create_dir_all(&dir).unwrap();
    let cases = [
        ("float.toml", "expected_version = 0.1", "expected_version"),
    ...
    std::fs::write(&p, "expected_version = \"1.0.0\"").unwrap();

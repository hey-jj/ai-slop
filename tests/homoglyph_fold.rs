...
//!
//! Without the fold, `dеlve` with a Cyrillic е (U+0435) exited CLEAN under
//! public-bug-report while the ASCII spelling fired SLOP-A001 — a reader
//! sees pixel-identical slop, the report is empty. The norm view folds
...
fn state_of<'r>(report: &'r ai_slop::Report, id: &str) -> Option<&'r str> {
    report
        .findings
        .iter()
        .find(|f| f.rule_id == id)
...
fn all_homoglyph_words_fire_at_candidate_tier() {
    ...
    let cases = [
        ("моѕаіс", "The design is a моѕаіс of ideas.", "SLOP-A001"),
        ("ерітоме", "It is the ерітоме of care.", "SLOP-A001"),
...
fn nfkc_compatibility_spellings_fire_hard() {
    ...
    for (label, src) in [
        ("fullwidth", "The design is a ｍｏｓａｉｃ of ideas."),
        ("math alphanumerics", "The design is a 𝗆𝗈𝗌𝖺𝗂𝖼 of ideas."),
    ] {
        let text = format!("{f}\n\n{src}\n", f = filler());
        let report = run(&text, Profile::PublicBugReport);
        assert_eq!(
